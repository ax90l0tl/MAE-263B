...
P_c1 = [L1/2;0;0;1];
P_c2 = [L2/2;0;0;1];
P_0_c1 = simplify(transpose(T_0_1)*P_c1);
P_0_c1 = P_0_c1(1:3);
P_0_c2 = simplify(transpose(T_0_2)*P_c2);
P_0_c2 = P_0_c2(1:3);

Z_0_1 = T_0_1(1:3, 3);
Z_0_2 = T_0_2(1:3, 3);
J_0_v1 = simplify([cross(Z_0_1, P_0_c1(1:3) - P_0_1), zeros(3, 1)]);
J_0_w1 = simplify([Z_0_1, zeros(3, 1)]);

J_0_v2 = simplify([cross(Z_0_1, P_0_c2(1:3) - P_0_1), cross(Z_0_2, P_0_c2(1:3) - P_0_2)]);
J_0_w2 = simplify([Z_0_1, Z_0_2]);
...
for j = 1:2
    for k = 1:2
        V_1 = simplify(V_1 + (diff(M(1, j), th(k)) - 0.5*diff(M(j, k), t1))*dq(j)*dq(k));
        V_2 = simplify(V_2 + (diff(M(2, j), th(k)) - 0.5*diff(M(j, k), t2))*dq(j)*dq(k));
    end
end
V = simplify([V_1;V_2]);
...
Tau_L = simplify(M*[t1dd;t2dd] + V + G);
Tau_L(1)
Tau_L(2)
...
wd_1 = transpose(R_0_1)*wd_0 + cross(w_1, t1d*[0;0;1]) + t1dd*[0;0;1];
...
vd_2 = simplify(transpose(R_1_2)*(cross(wd_1, P_1_2) + cross(w_1, cross(w_1, P_1_2)) + vd_1));
vd_2_c = simplify(cross(wd_2, P_c2) + cross(w_2, cross(w_2, P_c2)) + vd_2);
F_2 = m2*vd_2_c;
